...
def calculate_integrals(wf,PE=1,angle=0):
    integrals = []
    t_start = time.time()
    for i in range(len(wf)):
        listpeaks = search_peaks(wf[i],5,0.5,plot=False)
        #ints = su.integral_simulation_peaks(wfs[i],listpeaks,-5,0,50,100,5,8,10,plot=0,central=0)
        #integrals.append(ints)
        ints = integral_simulation_peaks(wf[i],listpeaks,-30,100,200,600,15,8,10,plot=0,central=1)
        integrals.append(ints)
